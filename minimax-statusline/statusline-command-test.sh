...
# Get context usage percentage (using raw input tokens, not cache)
context_size=$(echo "$input" | jq -r '.context_window.context_window_size')
input_tokens=$(echo "$input" | jq -r '.context_window.current_usage.input_tokens')
if [ -n "$context_size" ] && [ -n "$input_tokens" ] && [ "$context_size" -gt 0 ]; then
  used_pct=$((input_tokens * 100 / context_size))
